...
for list in listFiles:
    with open(list) as csvfile:
        ...
        for row in spamreader:
            if row[7] != "0":
                G.add_node(row[4], URL="https://etherscan.io/address/"+row[4], label=row[1], tooltip=row[4])
                G.add_node(row[5], URL="https://etherscan.io/address/"+row[5], label=row[1], tooltip=row[4])
...
graph.write_svg('output.svg', prog = 'circo', rankdir = "TB")
